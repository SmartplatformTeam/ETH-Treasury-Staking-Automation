...
이 문서가 `docs/`의 단일 source of truth다.
...
- 남음: 실제 `cluster.yml` / `hosts.yml` 기본값 연결, Web3Signer TLS/mTLS/auth/KMS 실연동, rollback, control plane approval/health API 직접 연결, script CI test.

## 8. Inventory 계약

`cluster.yml`은 cluster 공통값이고, `hosts.yml`은 host별 개별값이다.
현재 `render.sh`는 두 파일 경로를 명시적으로 넘기는 방식으로 쓴다.
...
- `.charon/charon-enr-private-key`: host별 artifact다. host 이름과 artifact source가 맞아야 한다.
...
## 12. Runtime automation 로컬 dry-run

실제 bare-metal host가 없어도 example inventory로 흐름을 검증할 수 있다.
...
로컬 dry-run용 artifact source 준비:

아래 `/tmp/cdvn-local-approved/operator-1`는 실제 운영 artifact가 아니라
stage 스크립트의 입력 계약을 확인하기 위한 dummy source다.
실제 rollout execute에는 이 dummy artifact를 쓰면 안 된다.

```bash
mkdir -p /tmp/cdvn-local-approved/operator-1/.charon
printf '{ "name": "local-dummy-cluster-lock" }\n' \
  > /tmp/cdvn-local-approved/operator-1/.charon/cluster-lock.json
printf 'local-dummy-enr-private-key-do-not-use\n' \
  > /tmp/cdvn-local-approved/operator-1/.charon/charon-enr-private-key
```

예시 inventory는 `web3signerFetch: true`라서 로컬 dry-run에는
`validator-pubkeys.txt`가 없어도 된다.

Artifact stage dry-run:
...
infra/obol-cdvn/scripts/stage-charon-artifacts.sh \
  ...
  --source-dir /tmp/cdvn-local-approved/operator-1
```
...
실제 값으로 수정한 뒤 `cluster.yml`, `hosts.yml` 경로를 render에 명시한다.
...
`/secure/approved-artifacts`는 보통 control plane 또는 secure staging host의 중앙 artifact 저장소다.
각 bare-metal host의 로컬 배포 경로가 아니다.
이 source dir 전체를 bare-metal host에 통째로 복사하지 않는다.
...
- 각 bare-metal host에는 자기 host에 해당하는 staged runtime만 rollout 된다.
...
이 시점의 bundle은 아직 deploy-ready가 아니다. `.charon` artifact, `jwt/jwt.hex`, host preflight가 남아 있다.

### 13.6 Artifact stage

각 host마다 dry-run을 먼저 본다.
...
예를 들어 `operator-1` stage는
`.tmp-cdvn-cluster-render/hosts/operator-1/runtime/.charon/`만 채운다.
`operator-2`부터 `operator-4` artifact가 `operator-1` runtime에 같이 들어가면 안 된다.
...
- staged `.charon` 파일이 있는가
- Web3Signer overlay인데 `.charon/validator_keys`가 들어가지 않았는가
- `WEB3SIGNER_FETCH=false`일 때 pubkey 입력이 준비됐는가

실패하면 rollout으로 넘어가지 않는다.

### 13.8 Rollout dry-run
...
Rollout 직전 실제 host 준비 상태를 확인한다.

이 시점에도 rollout 대상은 host 하나의 runtime 디렉토리뿐이다.
예를 들어 `operator-1` rollout은
`.tmp-cdvn-cluster-render/hosts/operator-1/runtime/`만
`operator-1`의 `deploymentPath`로 rsync 한다.
`/secure/approved-artifacts/` 전체나 다른 operator runtime을 같이 보내는 것이 아니다.
...
이 단계는 파일 전송 단계다. 아직 `docker compose up`을 실행한 것은 아니다.

### 13.11 Remote compose 실행
...
1. 4개 host 모두 render / stage / verify / rollout까지 끝낸다.
2. 4개 host 모두 preflight를 통과시킨다.
3. host를 하나씩 execute 한다.
...
6. 실제 `cluster.yml`, `hosts.yml` 분리와 기본값 연결
7. Web3Signer/KMS TLS, mTLS, auth header, KMS namespace 실연동
8. staged artifact remote hash 검증 추가
9. rollback / stop rule 고도화
10. `health-sync.sh`와 rollout approval을 control plane API와 연결
11. script smoke test와 CI 편입

Runtime 쪽 다음 작업 상세:

- 실제 4대 bare-metal host의 IP, DNS, SSH user, deployment path, monitoring port, cluster peer 값을 확정한다.
- `cluster.example.yml`, `hosts.example.yml`를 참고해 실운영 `cluster.yml`, `hosts.yml`를 secure path에 둔다.
...
- rendered bundle: inventory와 overlay를 반영해 host별로 만든 배포용 runtime 산출물.
...
```text
inventory -> artifact approval -> render -> artifact stage -> verify -> rollout dry-run -> preflight -> rollout execute -> compose execute -> validating 확인
```
