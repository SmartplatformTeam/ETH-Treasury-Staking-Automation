## Control Plane Application Rules

This product uses a Miro-inspired design language, but it is an operator control plane, not a marketing site. The Web UI must translate Miro's white canvas, confident black primary, canary yellow accent, and pastel status surfaces into dense operational screens.

### Contrast Tokens

Every filled surface must use an explicit foreground pair. Do not place a token on top of itself.

| Background Token | Foreground Token | Use |
|---|---|---|
| `--miro-primary` (`#050038`) | `--miro-on-primary` (`#ffffff`) | Active navigation, primary buttons, dark log surfaces |
| `--miro-yellow` (`#ffd02f`) | `--miro-on-yellow` (`#050038`) | Brand mark and yellow emphasis only |
| `--miro-blue` (`#4262ff`) | `--miro-on-blue` (`#ffffff`) | Rare blue action surfaces |
| `--miro-canvas` / `white` | `--miro-primary` or `--miro-charcoal` | Default page, panels, table cells |
| Pastel surfaces | Dedicated dark semantic text | Status badges and warning/error callouts |

Rules:
- A primary background must never use `--miro-primary` text. Use `--miro-on-primary`.
- A yellow background must never use muted gray text. Use `--miro-on-yellow`.
- Disabled controls use `--miro-hairline` background with `--miro-muted` text.
- Log surfaces may use `--miro-primary`, but all log text and metadata must use `--miro-on-primary` with opacity for hierarchy.

### Radius Scale For The App

Miro's pill shapes are part of the source inspiration, but this control plane should feel more compact and operational. Use moderate radius by default.

| Shape | Value | Use |
|---|---:|---|
| `rounded-md` | 6px | Badges, compact tags |
| `rounded-lg` | 8px | Small action links, log event rows |
| `rounded-xl` | 12px | Inputs, buttons, nav items, panels, run cards |
| `rounded-2xl` | 16px | Reserved for unusually large feature blocks only |
| `rounded-full` | 9999px | Avoid by default; use only when a true pill is intentionally required |

Rules:
- Primary buttons, select inputs, and nav items use `rounded-xl`, not `rounded-full`.
- Status badges use `rounded-md`.
- Cards and panels use `rounded-xl`.
- Avoid stacking multiple high-radius containers inside each other.

### Long Identifier Handling

Operational data includes public keys, cluster names, playbook paths, inventory refs, run IDs, and host names. These must not stretch cards or tables.

Rules:
- Table string cells render as single-line `truncate` values with the full value available via `title`.
- Tables use horizontal overflow on narrow viewports rather than squeezing columns until text overlaps.
- Detail fields such as Run ID, Playbook, Inventory, public keys, and cluster names must live in `min-w-0` containers and use `truncate`.
- Log output is the exception: logs use `whitespace-pre-wrap` and `break-words` inside a bounded scroll container.
- If a value is important for copying or audit review, prefer a dedicated detail row with truncation + hover title over wrapping through the whole card.

### Current Web Translation

- Canvas: white page background with soft off-white sidebar/panel contrast.
- Primary action: `--miro-primary` background + `--miro-on-primary` text.
- Brand accent: `--miro-yellow` logo tile only, with `--miro-on-yellow` text.
- Status: pastel badges with dark semantic foregrounds.
- Tables: flat white surface, subtle hairline borders, truncating default cells.
- Automation detail/logs: metadata truncates; log body wraps inside a bounded dark surface.
